...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            elif insect.can_contain(self.ant) and not self.ant.container:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        distance = 0
        if random_or_none(self.place.bees) != None and distance >= self.min_range and distance <= self.max_range:
        	return random_or_none(self.place.bees)
        p = self.place
        while p.entrance != hive:
        	distance += 1
        	if random_or_none(p.entrance.bees) != None and distance >= self.min_range and distance <= self.max_range:
        		return random_or_none(p.entrance.bees)
        	p = p.entrance
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if new_armor <= 0:
        	for item in list(self.place.bees):
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
        	...
        	while p:
        		if p.ant:
        			if p.ant not in QueenAnt.double_list:
        				p.ant.damage *= 2
        				QueenAnt.double_list += [p.ant]
        			if p.ant.container and p.ant.ant and p.ant.ant not in QueenAnt.double_list:
        				p.ant.ant.damage *= 2
        				QueenAnt.double_list += [p.ant.ant]
        		p = p.exit
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.armor -= amount
        if self.armor <= 0 and self.imposter:
            self.place.remove_insect(self)
        elif self.armor <= 0 and self.imposter == False:
        	self.place.remove_insect(self)
        	bees_win()
        # END Problem 13

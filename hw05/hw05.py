...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t) and label(t) == old:
        ...
        new_branch = [replace_leaf(i, old, new) for i in branches(t)]
...
def balanced(m):
    ...
    "*** YOUR CODE HERE ***"
    def torque(s):
        return label(s) * total_weight(end(s))
    if False in [balanced(end(i)) for i in sides(m) if not is_weight(end(i))]:
        return False
    torque_euqal = torque(sides(m)[0]) == torque(sides(m)[1])
    if not torque_euqal:
        return False
    return True
